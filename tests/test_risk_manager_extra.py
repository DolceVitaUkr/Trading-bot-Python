...
def test_learning_phase_uses_fixed_notional():
    ...
    size = rm.computesize(price=100.0, stop_loss=90.0)
    assert size == 10.0
...
def test_growth_phase_applies_tiers_and_weights():
    ...
    size = rm.computesize(price=100.0, stop_loss=90.0, signal_strength="strong")
...
def test_breach_check_flags_exposure_and_daily_loss():
    ...
    rm.registeropen(600.0)  # exposure > 50% of balance
    rm.balance = 850.0       # 15% loss from starting equity
    breaches = rm.riskbreachcheck()
