...
def strategy_manager(mock_data_provider):
    """Fixture to create a StrategyManager with a mock data provider."""
    # Mock the global config object that the strategies might depend on
    with patch('modules.Strategy_Manager.config') as mock_config:
        mock_config.OPTIMIZATION_PARAMETERS = {
            'trend_ema_period': [10, 20],
            'mr_bollinger_period': [20],
            'mr_bollinger_std': [2]
        }
        manager = StrategyManager(data_provider=mock_data_provider)
    return manager

def test_decide_no_signal(strategy_manager):
    """Tests that decide returns None when the strategy finds no signal."""
    strategy_manager.strategy_map['Trend'].check_entry_conditions = MagicMock(return_value={})

    context = {"regime": "Trend", "adx": 30}
...
def test_decide_trend_signal(strategy_manager):
    ...
    strategy_manager.strategy_map['Trend'].check_entry_conditions = MagicMock(return_value=mock_signal)

    context = {"regime": "Trend", "adx": 40}
    decision = strategy_manager.decide("BTC/USDT", context)
...
def test_good_setup_flag(strategy_manager):
    ...
    high_score_context = {"regime": "Trend", "adx": 55}
    ...
    low_score_context = {"regime": "Trend", "adx": 20}
