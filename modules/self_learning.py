...
class SelfLearningBot:
    ...
    def _define_strategy_variations(self) -> List[Dict]:
        ...
        based on the settings in config.OPTIMIZATION_PARAMETERS.
        """
        ...
        for values in itertools.product(*param_grid.values()):
            ...
            if params.get("ema_short_period_1", 0) >= params.get("ema_short_period_2", 1):
                continue
            if params.get("ema_short_period_2", 0) >= params.get("ema_long_period", 1):
                continue

            # Generate a unique ID for the strategy variation
            param_str = "_".join(f"{k.replace('_period', '')}{v}" for k, v in sorted(params.items()))
        ...
        logger.info(f"Generated {len(variations)} strategy variations for testing.")
        if not variations:
            logger.warning("No strategy variations were generated. Check config.OPTIMIZATION_PARAMETERS.")
            # Fallback to a default strategy to prevent errors
            default_params = {
                "ema_long_period": 50, "ema_short_period_1": 8, "ema_short_period_2": 21,
                "rsi_period": 14, "rsi_level": 50, "atr_period": 14,
                "tp_atr_multiplier": 2.0, "sl_atr_multiplier": 1.5
            }
            variations.append({"id": "default_fallback", "params": default_params})
    ...
    def load_best_strategy(self) -> Optional[Dict]:
        ...
        try:
            ...
            logger.warning(f"{STRATEGY_MEMORY_FILE} not found. Using default strategy.")
            return None
        except Exception as e:
    ...
    def _log_strategy_performance(self, results: Dict):
        ...
        try:
            ...
            logger.info(f"Logged {len(results)} strategy performance records to {log_file}")
    ...
    def train_and_select_strategy(self):
        ...
        It simulates each strategy variation defined in `_define_strategy_variations`
        for a fixed number of trades (100, as requested).
        It then evaluates the PnL of each variation and saves the best one to memory.
        """
        logger.info(f"Starting training process with {len(self.strategy_variations)} variations.")
        if self.ui_hook:
            self.ui_hook.log(f"Training started: {len(self.strategy_variations)} variations to test.", level="SUCCESS")
        ...
        for variation in self.strategy_variations:
            ...
            # Score rewards both PnL and consistency (win rate).
            # A simple multiplier; more advanced scoring could normalize metrics.
            score = pnl * win_rate

            results[strategy_id] = {"pnl": pnl, "win_rate": win_rate, "score": score, "params": params}
            log_msg = f"Strategy {strategy_id} finished with PnL: {pnl:.2f}, Win Rate: {win_rate:.2%}, Score: {score:.2f}"
            logger.info(log_msg)
            if self.ui_hook:
                self.ui_hook.log(f"  - {strategy_id} PnL: {pnl:.2f}, Win Rate: {win_rate:.2%}")
        ...
        logger.info(f"Training complete. Best strategy is {best_strategy_id} with Score: {best_result['score']:.2f} (PnL: {best_result['pnl']:.2f}, Win Rate: {best_result['win_rate']:.2%})")
        if self.ui_hook:
            self.ui_hook.log(f"Training complete. Best strategy: {best_strategy_id}", level="SUCCESS")
    ...
    def run(self):
        ...
        If `training` is True (set via UI or config), it will first run the
        `train_and_select_strategy` method. Once training is complete, it will
        disable the training flag and proceed to trade with the best-performing
        strategy it found.

        If `training` is False, it will immediately start trading with the last
        known best strategy loaded from `strategy_memory.json`.
        """
        if self.training:
            self.train_and_select_strategy()

        logger.info("Bot switching to trading mode with the best learned strategy.")
