...
class ParameterOptimizer:
    ...
    def __init__(self):
        ...
        self.checkpoint_file = getattr(
            config,
            "OPTIMIZATION_CHECKPOINT_FILE",
        ...
        for key, bounds in self.params.items():
            if isinstance(bounds, dict):
                self.toolbox.register(
                    f"attr_{key}", random.uniform, bounds["min"], bounds["max"])
            else:
    ...
    def optimize(self,
                 objective_function: Callable[[Dict], float]) -> Dict:
        """
    ...
    def run(self,
        ...
        """
        Deletes old checkpoint and runs optimization using a simple
        EMA crossover objective.
        """
        if os.path.exists(self.checkpoint_file):
            try:
        ...
        if initial_balance is None:
            initial_balance = getattr(
                config, "SIMULATION_START_BALANCE", 1000.0)
        ...
        def ema_crossover_objective(params: Dict) -> float:
            try:
                ...
                for i in range(1, len(df)):
                    prev_short = df["ema_short"].iat[i - 1]
                    prev_long = df["ema_long"].iat[i - 1]
                    curr_short = df["ema_short"].iat[i]
                    curr_long = df["ema_long"].iat[i]
                    price = df["close"].iat[i]

                    if (prev_short <= prev_long and
                            curr_short > curr_long and position == 0):
                ...
                logger.error(
                    f"Objective function error with params {params}: {e}")
                return -np.inf
    ...
    def _grid_search(self, objective_function: Callable) -> Dict:
        ...
        with ProcessPoolExecutor() as executor:
            futures = {
                executor.submit(
                    self._evaluate_parameters, objective_function, c): c
                for c in combos}
        ...
        for i in range(n_iter):
            params = {
                k: (random.choice(v) if isinstance(v, list)
                    else float(np.random.uniform(v["min"], v["max"])))
    ...
    def _evolutionary_optimization(self,
        ...
        self.toolbox.register(
            "evaluate", self._ea_evaluation_wrapper, objective_function)
        pop_size = int(getattr(config, "EA_POPULATION_SIZE", 20))
        ...
        crossover_prob = float(getattr(config, "EA_CROSSOVER_PROB", 0.5))
        mutation_prob = float(getattr(config, "EA_MUTATION_PROB", 0.2))
        generations = int(getattr(config, "EA_GENERATIONS", 10))
        algorithms.eaSimple(
            pop,
            self.toolbox,
            cxpb=crossover_prob,
    ...
    def _ea_evaluation_wrapper(self,
                               objective_function: Callable,
                               individual) -> tuple:
    ...
    def _select_best_result(self, results: List) -> Dict:
        ...
        df = pd.DataFrame(
            results,
            columns=["params", "score"]).sort_values(
    ...
    def _generate_parameter_combinations(self) -> List[Dict]:
        ...
        keys, values = zip(*[
            (k, v if isinstance(v, list)
             else list(np.linspace(v["min"], v["max"], num=10)))
            for k, v in self.params.items()])
