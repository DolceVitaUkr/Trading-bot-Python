...
class TradingUI(App):
    ...
    def _update_timeseries(self, *, wallet: Optional[float] = None, vwallet: Optional[float] = None, points: Optional[float] = None):
    ...
    def update_timeseries(self, *, wallet: Optional[float] = None, vwallet: Optional[float] = None, points: Optional[float] = None):
        if self.is_running:
            self.call_soon(self._update_timeseries, wallet=wallet, vwallet=vwallet, points=points)
    ...
    def _refresh_bot_status(self) -> None:
        ...
        self.query_one("#connection-status", Static).update(f"⚡ {'Connected' if is_conn else 'Disconnected'}")
        is_training = bool(getattr(self.bot, "training", False))
        self.query_one("#bot-mode", Static).update(f"Mode: {'Training' if is_training else 'Live' if getattr(self.bot, 'trading', False) else 'Idle'}")
        if hb_ts := getattr(self.bot, "last_heartbeat", None):
            self.query_one("#heartbeat-status", Static).update(f"Heartbeat: {datetime.fromtimestamp(hb_ts).strftime('%H:%M:%S')}")
        else:
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if action_id in self._action_handlers:
            try:
                ...
                asyncio.run(self.log(f"Action '{action_id}' failed: {e}", level='ERROR'))
        else:
            asyncio.run(self.log(f"No handler for action '{action_id}'", level='WARN'))
