...
def _toarray(data: Sequence[float]) -> np.ndarray:
...
def sharperatio(returns: Sequence[float], freq: int = 252) -> float:
...
def sortinoratio(returns: Sequence[float], freq: int = 252) -> float:
...
def maxdrawdown(equity: Sequence[float]) -> float:
...
def calmarratio(returns: Sequence[float], freq: int = 252) -> float:
    ...
    mdd = abs(maxdrawdown(np.cumprod(1 + r)))
    return float(cagr / mdd) if mdd != 0 else 0.0


def omegaratio(returns: Sequence[float], threshold: float = 0.0) -> float:
    r = _toarray(returns)
...
def profitfactor(pnl: Sequence[float]) -> float:
    p = _toarray(pnl)
    gains = p[p > 0].sum()
    losses = -p[p < 0].sum()
    return float(gains / losses) if losses != 0 else float("inf")
...
def winrate(pnl: Sequence[float]) -> float:
    p = _toarray(pnl)
    if p.size == 0:
        return 0.0
...
def averager(pnl: Sequence[float], risk_per_trade: float = 1.0) -> float:
    p = _toarray(pnl)
    if risk_per_trade == 0:
        return 0.0
...
def turnover(positions: Sequence[float]) -> float:
    pos = _toarray(positions)
    return float(np.abs(np.diff(pos, prepend=0)).sum())


def slippageerror(model: Sequence[float], realised: Sequence[float]) -> float:
    m = _toarray(model)
    r = _toarray(realised)
...
def holdingtimestats(durations: Sequence[float]) -> Dict[str, float]:
    d = _toarray(durations)
    if d.size == 0:
...
class BacktestEngine:
    ...
    # ------------------------------------------------------------------
    def _applymicrostructure(self, order: Dict[str, Any]) -> ExecutedTrade:
    ...
    def run(self, orders: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        trades: List[ExecutedTrade] = [self._applymicrostructure(o) for o in orders]
...
def montecarlobootstrap(pnl: Sequence[float], iterations: int = 1000) -> List[float]:
    ...
    for _ in range(iterations):
        ...
        res.append(sharperatio(sample, freq=1))
    return res


def slippagescenarios(trades: Sequence[ExecutedTrade], multipliers: Sequence[float]) -> Dict[str, float]:
...
class ValidationManager(ValidationRunner):
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def computemetrics(result: Dict[str, Any]) -> Dict[str, float]:
        pnl = result.get("pnl", [])
        equity = result.get("equity", [])

        returns = np.diff(equity, prepend=0)
        metrics = {
            "sharpe": sharperatio(returns, freq=1),
            "sortino": sortinoratio(returns, freq=1),
            "calmar": calmarratio(returns, freq=1),
            "omega": omegaratio(returns),
            "max_dd": abs(maxdrawdown(equity)),
    ...
    # ------------------------------------------------------------------
    def latestreport(self, strategy_id: str) -> Optional[Dict[str, Any]]:
    ...
    # ------------------------------------------------------------------
    def eligibleforlive(self, strategy_id: str) -> Tuple[bool, List[str]]:
        report = self.latestreport(strategy_id)
        if not report:
    ...
    async def approved(self, strategy_id: str, market: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        passed, reasons = self.eligibleforlive(strategy_id)
...
monte_carlo_bootstrap = montecarlobootstrap
slippage_scenarios = slippagescenarios
