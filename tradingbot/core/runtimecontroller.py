...
class RuntimeController:
    ...
    # ------------------------------------------------------------------
    def enablelive(self, asset: str) -> None:
    ...
    def disablelive(self, asset: str, close_only: bool = False) -> None:
        asset_state = self.state["assets"].setdefault(
            asset, {"live": False, "close_only": False, "consecutive_losses": 0}
        )
    ...
    def recordtraderesult(self, asset: str, is_live: bool, pnl_after_fees: float) -> None:
        if not is_live:
            return
        asset_state = self.state["assets"].setdefault(
            asset, {"live": False, "close_only": False, "consecutive_losses": 0}
    ...
    set_global_kill = setglobalkill
    record_trade_result = recordtraderesult
    get_state = getstate
